...
(function(){
  ...
  function initUI(){
    ...
    if (btnAddBall) btnAddBall.addEventListener('click', ()=>{ physics.addBallFromParams(collectParams()); });
  ...
  function bindCanvasSelection(){
    ...
    function hitTest(px, py, snap){
      ...
      if (snap.scenario === 'collision-1d') {
        ...
        // 检查主球
        const x = marginPx + snap.position.x * ppm; const y = canvas.height - marginPx - snap.position.y * ppm;
        if (Math.hypot(px - x, py - y) <= radiusPx) return {type:'main'};
        
        // 检查额外小球
        if (snap.balls && snap.balls.length > 0) {
          for (const ball of snap.balls) {
            const bx = marginPx + ball.x * ppm; const by = canvas.height - marginPx - ball.y * ppm;
            if (Math.hypot(px - bx, py - by) <= radiusPx) return {type:'ball', id: ball.id};
          }
        }
        return null;
      }
